  ...
  return (
    <div className="w-80 sm:w-88 md:w-96 h-full bg-obsidian-925 border-l border-obsidian-700/60 flex flex-col justify-between shadow-panel select-none z-20 animate-fade-in">
      ...
      <div>
        ...
        <div className="p-4 space-y-3 overflow-y-auto max-h-[calc(100vh-230px)]">
            ...
            {sections.length > 0 ? (
              <div className="space-y-1.5">
                {sections.map((sec, idx) => (
                  <div
                    key={sec.id || idx}
                    onClick={() => onScrollToLine && onScrollToLine(sec.startLineIndex)}
                    className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-obsidian-950 hover:bg-obsidian-900 border border-obsidian-700/50 hover:border-accent cursor-pointer transition-fast group"
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="text-[11px] font-mono text-obsidian-500 group-hover:text-accent">
                        {String(idx + 1).padStart(2, '0')}
                      </span>
                      <span className="text-xs font-mono font-semibold text-white group-hover:text-accent transition-fast">
                        {sec.title}
                      </span>
                    </div>

                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-obsidian-900 text-obsidian-400 border border-obsidian-800 group-hover:border-accent/40 group-hover:text-accent">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {SECTION_TEMPLATES.map((tmpl) => (
                <button
                  ...
                  className="flex items-center justify-between px-2.5 py-1.5 rounded bg-obsidian-950 hover:bg-obsidian-900 border border-obsidian-700/50 hover:border-obsidian-500 text-xs font-mono text-obsidian-300 hover:text-white transition-fast group text-left"
                >
                  <span className="truncate">{tmpl.label}</span>
                  <Plus className="w-3 h-3 text-obsidian-500 group-hover:text-accent opacity-0 group-hover:opacity-100 transition-fast" />
                </button>
              ))}
            </div>
          </div>
